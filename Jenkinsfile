pipeline {
    agent any  // Global agent for stages without specific agents
    environment {
        API_KEY = credentials('NEWS_API_KEY')  // Secret text or file
        DOCKER_IMAGE = 'rsrprojects/flask-news-app'
        IMAGE_TAG = 'test'
        DOCKERHUB_CREDS = credentials('DOCKER_CREDENTIALS')  // Username/password
        TF_API_TOKEN = credentials('TERRAFORM_CLOUD_API')   // Secret text
        WORKSPACE_ID = 'ws-sbPFYMrFfwvtFurY'
        GITHUB_TOKEN = credentials('GITHUB_TOKEN')          // Secret text
        d_branchName = 'master'
    }
    stages {
        stage('Checkout') {
        ...
        stage('Install jq') {
            agent { docker 'ubuntu:20.04' }  // Use a container to avoid privilege issues
            steps {
                sh '''
